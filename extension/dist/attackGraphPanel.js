...
exports.showAttackGraphPanel = showAttackGraphPanel;
...
function normalizeSurfaceNodes(attackSurface) {
    ...
    return nodes
        ...
        label: stringValue(node.label, stringValue(node.node_id, "Unknown node")),
...
function normalizeThreats(threatModel) {
    ...
    return threats
        ...
        title: stringValue(threat.title, stringValue(threat.threat_id, "Attack path")),
        category: stringValue(threat.category, "unknown"),
        severity: stringValue(threat.severity, "unknown"),
        exploitability: stringValue(threat.exploitability, "unknown"),
...
function nodeCard(nodeId, nodes) {
...
function evidenceList(evidence) {
    ...
    return [
        ...
        ...evidence.slice(0, 8).map((item) => `<li>${escapeHtml(item)}</li>`),
        "</ul>",
    ].join("");
}
function crossingSection(path, crossings) {
...
function exposureSection(path, exposures) {
    const items = exposures.filter((item) => item.path_id === path.path_id);
    if (items.length === 0) {
        return "";
    }
    return [
        ...
        ...items.map((item) => [
            ...
            '<div>',
            `<span class="sentinel-class">${escapeHtml(item.data_classes
                .map(humanize)
                .join(" · "))}</span>`,
            `<p>Flow reaches ${escapeHtml(humanize(item.sink_kind))}</p>`,
            "</div>",
            `<span class="risk-pill ${riskClass(item.risk)}">${escapeHtml(item.risk.toUpperCase())}</span>`,
            "</div>",
        ].join("")),
        "</div>",
    ].join("");
}
function pathCard(path, index, nodes, threats, crossings, exposures) {
    ...
    return [
        ...
        `<span class="risk-pill ${riskClass(path.risk)}">${escapeHtml(path.risk.toUpperCase())}</span>`,
        `<span class="proof-pill">${escapeHtml(humanize(path.exploitability))}</span>`,
...
function sentinelSection(exposures, nodes) {
...
function renderHtml(webview, artifact, attackSurface, threatModel) {
    const nodes = new Map(normalizeSurfaceNodes(attackSurface)
        .map((node) => [node.node_id, node]));
    const threats = new Map(normalizeThreats(threatModel)
        .map((threat) => [
        threat.threat_id,
        threat,
    ]));
    const crossings = new Map(artifact.boundary_crossings.map((item) => [
        item.crossing_id,
        item,
    ]));
    const criticalOrHigh = artifact.summary.critical_paths
    ...
    const paths = artifact.attack_paths.length === 0
        ...
        : artifact.attack_paths
            .map((path, index) => pathCard(path, index, nodes, threats, crossings, artifact.sensitive_data_exposures))
            .join("");
    const csp = [
        "default-src 'none'",
        "img-src data:",
...
<main class="shell">
  ...
  ${sentinelSection(artifact.sensitive_data_exposures, nodes)}
...
function showAttackGraphPanel(artifact, attackSurface, threatModel) {
    const panel = vscode.window.createWebviewPanel("aegis.attackGraph", "Aegis · Attack Graph / Data Sentinel", vscode.ViewColumn.Beside, {
        enableScripts: false,
        retainContextWhenHidden: true,
    });
    panel.webview.html = renderHtml(panel.webview, artifact, attackSurface, threatModel);
}
//# sourceMappingURL=attackGraphPanel.js.map